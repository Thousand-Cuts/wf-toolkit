...
Use case: you're authoring a new dropdown and want to seed its options from an existing dropdown on the same tenant (or a different one).

## Scenario A — same tenant

> Consultant: "Add a 'Backup Region' dropdown to the disaster recovery form. Use the same options as the existing 'Primary Region' dropdown."
...
## Scenario B — cross-tenant

> Consultant: "Use the same country dropdown options we have on our reference tenant, applied to the client's tenant."

Same procedure but the GET runs against the source tenant; sanitiser strips the source `parameterID` reference (which doesn't apply to the destination) before the bulk-POST is composed for the destination's new parameter.
...
./skills/workfront-api/scripts/wf-curl.sh /attask/api/v17.0/parameterOption/search \
  ...
  > /tmp/firm-country-options.json

# 3. Switch to dest
./skills/workfront-api/scripts/wf-use.sh acme-tenant

# 4. Create the new parameter on dest
# (Skill collects displayName + parameterType from consultant)
...
- displayOrder is preserved from source unless the consultant overrides.

## What this demonstrates

- Existing tenant option lists are reusable for new dropdowns — no retyping.
- Cross-tenant cloning works for individual fields too, not just whole forms.
