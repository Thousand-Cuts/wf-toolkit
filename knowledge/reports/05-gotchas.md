...
**What the skill does:** before writing the REPORT row, walk every `column.N.valuefield` and `column.N.valueexpression` in the view definition, every `key=value` line in the filter definition, and every `group.N.valuefield` in the groupBy definition. For each leading token (the part before any `:`), check that it exists on `/<uiObjCode>/metadata`. Print a warning per miss. Do not block — the admin may know about a deliberate cross-object reference the metadata doesn't surface — but make sure the admin sees the warning before the `apply` gate.
...
When a report's filter references a custom-form data extension (`DE:Project Tier`, `DE:Region`, etc.), admins sometimes try to set `categoryID` on the REPORT row to "make the form available to the report." This does not do what it looks like it does.
...
Pair with the parity check from `03-clone-and-adapt-recipe.md` Phase 5 for the cross-environment case.

---

## 3. Subscriptions and dashboards are NOT touched by modify

If the admin modifies a report that has subscriptions (`SCHREP` — scheduled report deliveries) or is embedded in dashboards (`PTLSEC` — portal sections referencing the report), the modify writes against the underlying UIFT/UIGB/UIVW/REPORT rows and leaves those external references in place. The next subscription delivery uses the modified report. That's usually the desired behaviour — but the admin should know.
...
This is a soft warning, not a block. The admin types `apply` to proceed.
...
The bare URL opens the report's editor; the `/view` suffix renders the report immediately. The skill prints both after every create or modify so the admin has one click into either view.
...
**Side effects:**
- The orphaned UIFT/UIGB/UIVW row the skill just created is still in the tenant, unreferenced. The admin may want to delete it.
...
## 6. Cross-environment locale leak

Hard-coded dates in source `definition` strings carry the source environment's timezone interpretation. Example: a filter line `plannedCompletionDate=2026-04-01\nplannedCompletionDate_Mod=lte` in a source environment configured for `America/Chicago` means "before 2026-04-01 23:59:59 CDT". The same line on a destination environment configured for `Europe/London` means "before 2026-04-01 23:59:59 BST" — different absolute instant.
...
**What the skill does:** during clone-flow Phase 4 sanitisation, every hard-coded date inside any `definition` string lands on the `prompt` list. The admin chooses one of:
...
**When this matters:** the modify flow. If the admin wants to "change the filter on report X" and the skill PUT-in-place against the UIFT, every other report referencing that UIFT also gets the new filter.
...
The default is option 2 — preserve the other consumers — but the admin decides.

---

## 8. Custom-form parameter parity in clone

A `DE:<name>` reference inside any source `definition` string works on the destination environment if and only if the destination has a custom field with the exact same name on the same target object. Names are case-sensitive. Spaces matter. Trailing whitespace matters.
...
> "`DE:<name>` does not exist on `<customer-name-dst>`. Either create the custom field on the destination environment before re-running, or remove the column / filter line that references it. Type `edit` to revise, anything else to abort."

The skill never auto-strips a `DE:` reference — the admin's intent matters too much. A missing custom field is almost always a real problem to solve, not a value to silently drop.

**Cross-form ambiguity.** A parameter name may exist on multiple custom forms on the destination. The parity check surfaces the count and the form names; if there's exactly one, it's a match. If there's more than one, the skill warns that the destination has multiple `<name>` parameters and asks the admin to verify the correct form is attached to the target objects being reported on.
...
**Empirical evidence:** This was the first error caught in the live v0.8.0 test on 2026-05-13. The "Active Projects" report (`aaaa1111bbbb2222cccc3333dddd4444` on a live Workfront sandbox) was created with a filter that included `isTemplate=false`; the UI rendered an error.
...
When this report is cloned from Client D to a different tenant (Client E, say), the `https://client-d.my.workfront.com/...` literal goes along for the ride — clicking the link in the cloned report opens Client D's tenant, not the destination's. In the promotion flow, the same leak sends prod report viewers to your preview sandbox.

**What the skill does:** During clone-flow Phase 4 sanitization, the walker scans every `valueexpression` for `https://[a-z][a-z0-9-]+(\.sb\d+)?\.(my|preview)\.workfront\.com/` patterns. Matches land in the new `host_rewrite` bucket. The interactive review at Phase 5 asks per-entry:

- **Auto-rewrite** — substitute the source host with the destination host (default for most cases)
- **Keep** — leave as-is (rare; only if pointing at a third-party asset that's the same across environments)
...
- When the skill CREATES a report, `preferenceID` is auto-minted by Workfront. The report renders correctly as a TABLE. Any chart the admin requested via interview is documented but not persisted; the admin must open the report in the in-product builder and configure the chart there.
- When the skill CLONES a report from a source environment to a destination environment, the preferenceID points at a source-environment resource that doesn't exist on the destination. The cloned report renders correctly as a table; charts and prompts that existed on the source are silently NOT carried over.

**Mitigation:** v0.9.0's recipes explicitly route admins asking for charts/prompts to "set the boolean flag, finish configuration in the in-product builder." See `02-create-from-scratch-recipe.md` § Chart and Prompts.

**v0.10.0 follow-up:** Probe the `preferenceID` resource against Client C's 2 chart reports to figure out the round-trip. Tracked on the roadmap.

---

## 13. Pseudo-fields: real-at-runtime, missing from `/metadata`

Some Workfront query-time field names are **real and accepted by the API** but DO NOT appear in the corresponding `/<uiObjCode>/metadata` response. The pre-flight validator (v0.9.0) blocks them as "no such field on `<uiObjCode>`" — a false positive that forces the admin to either edit the bundle or override pre-flight.

**Empirical examples (discovered during the v0.9.1 live test against a live Workfront sandbox, 2026-05-14):**
...
**v0.11.0+ (self-learning, analog of `workfront-api`'s `[wf-api-verify]` flow):** the validator gains a `--learn` flag; when an admin says "this field works, trust me" after a pre-flight block, the field name is recorded in a per-tenant whitelist (`~/.cache/wf-toolkit/reports-pseudo-fields-<host-hash>.json`) so future runs against the same tenant accept it. Mirrors `workfront-api`'s self-teaching loop (`skills/workfront-api/SKILL.md` § 92 and `knowledge/api/13-local-verification.md`) where verified gaps land back in the skill knowledge after the admin confirms.
...
**Pre-flight detection (v0.16.1+):** `pre_flight_validator.py` scans every `valueexpression`, `aggregator.valueexpression`, and `group.valueexpression` string in the bundle for the four curly chars and emits a hard error per match — with the path, the offending value, and a sed-fix suggestion. The lint runs BEFORE the brace-reference scan since a curly-quoted expression is syntactically broken anyway. The authoring interview still echoes composed expressions back for visual confirmation as a belt-and-braces check; the lint catches the case where an admin pastes a pre-built expression past the interview.
...
**Cross-skill note.** This is properly a `workfront-calc-fields` concern; the gotcha is restated here because the symptom surfaces FIRST in a report.
...
**Skill behaviour.** A reports-flow query that filters or groups on `actualCompletionDate` and aims at "first completion" semantics should prompt the admin: "actualCompletionDate is sticky across reopen/reclose; if you need the original first-complete date, use a custom captured field instead." The skill does NOT auto-substitute (the admin may genuinely want last-completion semantics).
...
Adobe's Aug 2025 Skill Exchange session confirms this is a known engine limitation, not a configuration bug: "if you're using text mode in your groupings, it's probably going to break your charts and the summary page". The Workfront chart renderer expects a flat, server-resolvable field reference for its group axis; valueexpression-grouped buckets are resolved post-aggregation and the chart layer doesn't see them.
...
**Skill behaviour.** When the admin requests a grouped report AND specifies "with a chart" or "with the summary view enabled," and the grouping logic is non-trivial (range buckets, conditional categories, etc.), the interview offers two paths: (a) text-mode-grouped report WITHOUT a working chart/summary, or (b) custom-form-field-backed grouping WITH a working chart/summary. The admin picks; the skill does not silently choose. Cross-link to `workfront-calc-fields` for the field setup.
