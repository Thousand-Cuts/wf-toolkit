...
- **Cross-tenant: never write to source.** In the clone flow, source and destination are pre-registered as separate client folders. The skill activates the source slug for read phases and the dest slug for write phases via `/wf-client-use`. The wrapper sources `~/wf-clients/.active`, so cross-instance leakage is impossible by design. Every interactive step prints a banner naming both tenants. The single `apply` gate names the destination tenant explicitly.
- **API key is never in chat or context.** Credentials live in `~/wf-clients/<slug>/.env` at mode 600, set by the consultant via `wf-client-setkey.sh` in their terminal. Every API call against a client tenant goes through `bash ${CLAUDE_PLUGIN_ROOT}/skills/_shared/scripts/wf-client-curl.sh` — the wrapper sources the active client's .env and puts `apiKey=` in the URL query string (no key in argv).
- **Prod destination requires explicit acknowledgement.** When the active (destination) client is `WF_ENV_TYPE=prod`, the wrapper refuses every write (exit 3) until `WF_CLIENT_WRITE_ACK=1` is set per call. After the `apply` gate, surface the prod warning verbatim, get a typed `yes`, then prepend the env var to every wrapper invocation in the 3-or-4-call write sequence.
...
For runs the consultant wants to persist (smoke-test JSON, the success URL printout, clone sanitization reports), the recipes write them to `~/wf-clients/<dest-slug>/deliverables/<UTC>-report-<verb>.{json,md}`. See `02-create-from-scratch-recipe.md` Phase G and `03-clone-and-adapt-recipe.md` for the exact paths.
