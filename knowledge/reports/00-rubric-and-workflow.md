...
The `workfront-reports` skill creates new Adobe Workfront reports and modifies existing ones via the REST API. Two flows: **NL-create** authors a report from a natural-language brief; **clone-and-adapt** lifts a known-good report from a source environment and writes a sanitised copy into a target environment (e.g. sandbox → prod). This file is the dispatch sheet — when the skill should run, when it should defer, the four-call REST sequence every create follows, and the safety baseline that applies to every run.
...
- "Clone the X report from the sandbox into prod."
- "Lift and shift this report to prod."
...
In short: any request that ends in a written REPORT row (or its UIFT / UIGB / UIVW siblings) inside a target Workfront environment.

## When NOT to use this skill

Route elsewhere if the admin's actual need is one of these:

- **In-product text-mode tweak the admin can paste themselves** (no API write needed) → `workfront-textmode`. If the admin has the report open in the in-product builder and wants help writing the `valueexpression` for one column, no API call is required — they paste into the Text Mode tab and save.
- **Ad-hoc query against the REST surface** (`/project/search`, `/task/search`, …) where no report row gets created → `workfront-api`. The reports skill always creates or mutates a `REPORT` row; if the admin only wants the data, the underlying object's `/search` is the right tool.
- **Write operations against data records** (projects, tasks, hours, users) — the reports skill writes report *definitions*, not the data the reports show → `workfront-api` for direct record writes.
...
- **Fusion scenarios** that read or write Workfront — none of that goes through the REST report endpoint; Adobe Workfront Fusion is out of scope for this toolkit.
...
### Clone-and-adapt (cross-environment)

The admin points at a known-good report living somewhere — usually the sandbox or preview environment the admin already has registered as an environment folder. The skill GETs the report and its three referenced UI-objects, sanitises environment-specific IDs/dates, flags `DE:<name>` custom-field references for parity check against the destination environment, applies any requested mutations, then writes into the destination on a single typed `apply`. See `03-clone-and-adapt-recipe.md` for the full procedure.

## Decision tree

```
Is there a similar known-good report in a source environment (sandbox/preview) the admin
...
Does the admin want to change an existing report?
...
dashboard composition?
  yes → out of scope for v1. Surface the limitation, complete what you can,
        point the admin at the in-product builder for the rest.
```
...
- **`definition` is a JSON object.** Each of UIFT, UIGB, UIVW has a `definition` field whose value is a structured object — NOT the text-mode `\n`-separated string an admin sees in the in-product Text Mode tab. See `06-filter-patterns.md` (filter half), `07-view-patterns.md` (view + group halves).
...
The `workfront-reports` skill writes to the destination Workfront environment. The design choice (per the spec) is "just write it" — reports are cheap to delete, so heavy multi-stage safety machinery (pre-state files, audit logs, rollback plans) is not warranted. The baseline:

- **One `apply` confirmation gate per run.** Not two. Not per-call. The admin types the literal word `apply` once to authorise the whole four-call sequence (or the PUT sequence on modify). No other word proceeds. `y`, `yes`, `proceed`, `apply now` — none of those count. If the admin types `edit`, return to the interview with the existing fields prefilled.
- **No backup-and-rollback machinery.** There is no pre-state JSON file, no audit log, no per-record rollback plan. The modify flow GETs the current state of the report + its three UI-objects and prints them inline before writing — *that* is the rollback mechanism. If the admin needs to revert, they copy the pre-state JSON from terminal scrollback and re-issue it as a PUT.
- **Cross-environment: never write to source.** In the clone flow, source and destination are pre-registered as separate environment folders. The skill activates the source slug for read phases and the dest slug for write phases via `/wf-env-use`. The wrapper sources `~/wf-envs/.active`, so cross-instance leakage is impossible by design. Every interactive step prints a banner naming both environments. The single `apply` gate names the destination environment explicitly.
- **API key is never in chat or context.** Credentials live in `~/wf-envs/<slug>/.env` at mode 600, set by the admin via `wf-env-setkey.sh` in their terminal. Every API call against an environment goes through `bash ${CLAUDE_PLUGIN_ROOT}/skills/_shared/scripts/wf-env-curl.sh` — the wrapper sources the active environment's .env and puts `apiKey=` in the URL query string (no key in argv).
- **Prod destination requires explicit acknowledgement.** When the active (destination) environment is `WF_ENV_TYPE=prod`, the wrapper refuses every write (exit 3) until `WF_ENV_WRITE_ACK=1` is set per call. After the `apply` gate, surface the prod warning verbatim, get a typed `yes`, then prepend the env var to every wrapper invocation in the 3-or-4-call write sequence.
- **`[wf-reports-verify]` flow is separate.** That flow is exclusively for sanity-checking the skill's documented behaviour against your own Workfront sandbox before reporting a divergence (see the closing phase below) — it runs against a sandbox credential registered via `/wf-env-add` and uses the `[wf-reports-verify]` prefix. See `09-verification-flow.md`. Don't conflate it with the main reports flow.
- **No `uiObjCode` mutation.** A report's `uiObjCode` is the object it reports on (PROJ, TASK, OPTASK, USER, …). Changing it after the fact is destructive — every column in the view becomes invalid because column tokens are resolved against the target object. If the admin requests a `uiObjCode` change on modify, hard-block and recommend delete-and-recreate. See `05-gotchas.md` #1.
- **Schema discovery before first write.** Before writing to any environment the skill hasn't seen this session, run the four-call `/metadata` burst (see `04-runtime-schema-discovery.md`). The REPORT object's field schema is not reliably published; runtime discovery is what protects the skill from `uiObjCode`-vs-`reportObjCode` drift.
...
If an admin tells the skill their instance is on a newer version (e.g., "we're on 22.3"), the skill may switch — but defer that decision to `workfront-api`. This knowledge bucket assumes `v17.0` everywhere.

## Output artifacts per run

None mandatory. This skill does not write plan / pre-state / audit files to disk by default.

What the skill *does* print to terminal scrollback:

- The four resolved JSON payloads before the `apply` gate (so the admin can copy them if they want a record).
...
- The smoke-test GET response: `GET /report/<id>?fields=*,definition`, so the admin can verify the report row references the UI-objects the skill just created (see the silent-re-resolution gotcha, `05-gotchas.md` #5).
...
For runs the admin wants to persist (smoke-test JSON, the success URL printout, clone sanitization reports), the recipes write them to `~/wf-envs/<dest-slug>/exports/<UTC>-report-<verb>.{json,md}`. See `02-create-from-scratch-recipe.md` Phase G and `03-clone-and-adapt-recipe.md` for the exact paths.

## Closing phase: surface divergences

If at any point during a run the live API behaviour or a peer report's structure diverged from what this knowledge bucket documents — a field shape mismatch, a missing required field on a POST body, a `valueexpression` pattern that worked in production but is described as impossible here, a new gotcha that bit the run before the `apply` gate caught it — the skill MUST surface that divergence to the admin before closing the session.

Concretely, before reporting "done":

1. Trust the observed behavior for the task at hand, and treat the divergence as possibly environment-specific (Workfront version, package, or configuration).
2. Name the file the divergence belongs in (`knowledge/reports/05-gotchas.md`, `knowledge/textmode/07-combined-and-shared-columns.md`, etc. — usually the file that gave wrong or incomplete guidance).
3. If it looks globally true, offer to draft a GitHub issue at https://github.com/Thousand-Cuts/wf-toolkit/issues with the endpoint, API version, date, and observed-vs-documented behavior.
4. Never edit the installed plugin's files.

This is what keeps the skill from rotting against the live API. The admin has the context to judge whether a one-off observation generalises; surface it explicitly rather than silently moving on.
...
- The cross-environment clone-and-adapt procedure: `03-clone-and-adapt-recipe.md`.
